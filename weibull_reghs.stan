...
parameters { 
  ...
  real<lower = 0.0, upper = 10000.00> sigma;
}
...
transformed parameters {
  ...
  real<lower = 0.0> scale_glob;
  
  c = sqrt(c2)*slab_scale;
  lambda_tilde = sqrt(c^2 * square(lambda)) ./ (c^2 + tau^2 * square(lambda));
  
  beta = z .* lambda_tilde*tau;
  scale_glob = p/(M-p)*(sigma/sqrt(N));
...
model {
  // priors
  lambda ~ cauchy(0, 1); //student_t(nu_local, 0, 1.0);  //
  z ~ normal(0, 1);
  c2 ~ inv_gamma(0.5*slab_df, 0.5*slab_df);

  tau ~ cauchy(0, scale_glob^2);//cauchy(0, 1);// normal(0, scale_global^2);//
